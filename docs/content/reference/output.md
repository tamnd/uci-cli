...
describes what users get. Pick a format with `-o`, or let uci choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
uci <command> -o table   # aligned columns for reading
uci <command> -o jsonl   # one JSON object per line, for piping
uci <command> -o json    # a single JSON array
uci <command> -o csv     # spreadsheet friendly
uci <command> -o tsv     # tab-separated
uci <command> -o url     # just the URL column
uci <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
uci <command> --fields id,title,url
```
...
```bash
uci <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
uci <command>            # a table, because this is a terminal
uci <command> | wc -l    # JSONL, because this is a pipe
```
